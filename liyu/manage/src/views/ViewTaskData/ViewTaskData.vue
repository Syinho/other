    ...
    <el-form>
        <el-form-item label="体测项目选择">
            ...
            <el-button-group style="margin-left: 20px">
                <el-button @click="choiceAllFields" type="success">
                    <i class="iconfont icon-true" style="margin-right: 3px"></i>
                    一键全选
                </el-button>
                <el-button type="danger" @click="cancelAllFields">
                    一键取消
...
const handleData = function (data) {
    const first_keys = Object.keys(data)
    first_keys.forEach(item => {
        arr.push({ name: item, value: [] })
        const second_keys=Object.keys(data[item])
    })
    console.log(arr)

}
handleData(queryResult.value)
...
/*  */
const choiceAllFields = function () {
    form.fields = []
    Array.prototype.forEach.call(options_fields.value, fields => {
        form.fields.push(fields.value)
    })
}
const cancelAllFields = function () {
    form.fields = []
}
const a = computed(() => queryResult.value)
watch(a, newVal => {
    console.log(newVal)
    handleData(newVal)
})
